...
void Model::Centralization() {
	...
	Move(x_offset, Axis::X);
	Move(y_offset, Axis::Y);
	Move(z_offset, Axis::Z);
}
...
void Model::Move(double value, Axis axis) {
	...
	for (int i = 0; i < limit; ++i) {
		vertices_[i * 3 + static_cast<unsigned int>(axis)] += value;
	}
}
...
void Model::Rotate(double value, Axis axis) {
	...
	for (size_t i = 0; i < vertices_count_; ++i) {
		double first_axis = vertices_[i * 3 + first_modifier];
		double second_axis = vertices_[i * 3 + second_modifier];
		vertices_[i * 3 + first_modifier] = cos(value) * first_axis - sin(value) * second_axis;
		vertices_[i * 3 + second_modifier] = sin(value) * first_axis + cos(value) * second_axis;
	}
}
